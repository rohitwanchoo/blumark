    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if(!$hasIsoName)
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
                ...
                <a href="{{ route('profile.edit') }}" class="inline-flex items-center px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white font-medium rounded-lg transition-colors">
        ...
        @else
            <form action="{{ route('distributions.store') }}" method="POST" enctype="multipart/form-data" x-data="distributionForm()" class="space-y-6">
                @csrf
                            ...
                            <div class="border-2 border-dashed rounded-xl p-8 text-center transition-all duration-200 cursor-pointer"
                                ...
                                <template x-if="files.length === 0">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-xs text-gray-500 mt-1">Upload multiple PDFs - Maximum {{ config('watermark.max_upload_mb') }}MB per file</p>
                                    </div>
                                </template>
                            </div>
                        ...
                        <div class="mb-4 flex items-center justify-between">
                            <label class="flex items-center">
                                <input type="checkbox"
                                       ...
                                       :checked="selectedLenders.length === {{ $lenders->count() }}"
                            ...
                            @foreach($lenders as $lender)
                                <label class="flex items-center p-4 hover:bg-gray-50 cursor-pointer">
                                    ...
                                    <div class="ml-4 flex-1">
                                        <p class="text-sm font-medium text-gray-900">{{ $lender->company_name }}</p>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input type="number"
                                       name="font_size"
                                       id="font_size"
                                       value="{{ old('font_size', config('watermark.defaults.font_size', 15)) }}"
                                       min="8"
                                       max="72"
                                       class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 focus:border-primary-500 focus:ring-primary-500 focus:outline-none">
                            </div>
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    <input type="color"
                                           name="color"
                                           id="color"
                                           value="{{ old('color', config('watermark.defaults.color', '#878787')) }}"
                            ...
                            <div>
                                ...
                                <input type="number"
                                       name="opacity"
                                       id="opacity"
                                       value="{{ old('opacity', config('watermark.defaults.opacity', 10)) }}"
                                       min="1"
                                       max="100"
                                       class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 focus:border-primary-500 focus:ring-primary-500 focus:outline-none">
                            </div>
                    ...
                    <div class="p-6">
                        @if($emailTemplates->isEmpty())
                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                                ...
                                <a href="{{ route('email-templates.create') }}" class="text-sm text-primary-600 hover:text-primary-800 font-medium">
                                    Create Template
                                </a>
                            </div>
                        ...
                        @else
                            <div class="space-y-3">
                                ...
                                @foreach($emailTemplates as $template)
                                    <label class="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
                                        <input type="radio" name="email_template_id" value="{{ $template->id }}" class="text-primary-600 focus:ring-primary-500">
                                        <div class="ml-3 flex-1">
                                            ...
                                            <p class="text-xs text-gray-500">Subject: {{ Str::limit($template->subject, 50) }}</p>
                                        </div>
                                    </label>
                                @endforeach
                            </div>
                            <div class="mt-3 text-right">
                                <a href="{{ route('email-templates.index') }}" class="text-sm text-primary-600 hover:text-primary-800">
                                    Manage Templates
                                </a>
                            </div>
                        @endif
                    ...
                    <div class="p-6">
                        @if($smtpSettings->isEmpty())
                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                                ...
                                <a href="{{ route('smtp-settings.index') }}" class="text-sm text-primary-600 hover:text-primary-800 font-medium">
                                    Configure SMTP
                                </a>
                            </div>
                            ...
                            <div class="space-y-3">
                                ...
                                @foreach($smtpSettings as $smtp)
                                    <label class="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
                                        ...
                                        <div class="ml-3 flex-1">
                                            <div class="flex items-center">
                                                <p class="text-sm font-medium text-gray-900">{{ $smtp->name }}</p>
                                                @if($smtp->is_active)
                                                    <span class="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">Active</span>
                                                @endif
                                                @if($smtp->provider)
                                                    <span class="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">{{ ucfirst($smtp->provider) }}</span>
                                                @endif
                                            </div>
                                            <p class="text-xs text-gray-500">{{ $smtp->host }}:{{ $smtp->port }} - From: {{ $smtp->from_email }}</p>
                                        </div>
                                    </label>
                                @endforeach
                            </div>
                            <div class="mt-3 text-right">
                                <a href="{{ route('smtp-settings.index') }}" class="text-sm text-primary-600 hover:text-primary-800">
                ...
                <div class="flex justify-end gap-4">
                    <a href="{{ route('distributions.index') }}" class="px-4 py-2 text-gray-700 hover:text-gray-900 font-medium">
                        Cancel
                    </a>
                    <button type="submit"
                    ...
                    return {
                        ...
                        toggleAll(event) {
                            if (event.target.checked) {
                                this.selectedLenders = @json($lenderIds);
                            } else {
